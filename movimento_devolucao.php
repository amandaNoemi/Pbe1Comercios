...
if ($id > 0) {
    ...
    if ($mov && isset($mov['idProduto'])) {
        ...
        // só continua se produto existir
        $sql = "SELECT id FROM produto WHERE id = ?";
        $stmt = mysqli_prepare($conexao, $sql);
        mysqli_stmt_bind_param($stmt, "i", $idProduto);
        mysqli_stmt_execute($stmt);

        $produtoExiste = mysqli_fetch_assoc(mysqli_stmt_get_result($stmt));
        ...
        if ($produtoExiste) {
            ...
            $sql = "UPDATE movimento SET dataDevolucao = NOW() WHERE idMovimento = ?";
            $stmt = mysqli_prepare($conexao, $sql);
            mysqli_stmt_bind_param($stmt, "i", $id);
            mysqli_stmt_execute($stmt);
        }
